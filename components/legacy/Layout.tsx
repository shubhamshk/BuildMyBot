...
const Layout: React.FC<LayoutProps> = ({ children, showNav = true }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background-light dark:bg-background-dark selection:bg-primary selection:text-white">
      {showNav && (
        <nav
          className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${scrolled ? 'py-3 glass shadow-lg' : 'py-5 bg-transparent'
            }`}
        >
...
const NavLink = ({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) => (
  <button
    onClick={onClick}
    className={`text-sm font-bold tracking-tight transition-colors hover:text-primary ${active ? 'text-primary' : 'text-slate-500 dark:text-slate-400'
      }`}
  >
    {label}
  </button>
