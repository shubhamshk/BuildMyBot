            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormInput 
                  ...
                  value={character.age}
                  error={errors.age}
                  onChange={(v) => setCharacter({...character, age: v})}
                />
                <FormSelect 
                  label="Gender Identity" 
                  value={character.gender}
                  options={['Female', 'Male', 'Non-binary', 'Mystical']}
                  onChange={(v) => setCharacter({...character, gender: v})}
                />
              </div>
              ...
              <FormInput 
                ...
                onChange={(v) => setCharacter({...character, setting: v})}
              />
              ...
              <FormInput 
                ...
                onChange={(v) => setCharacter({...character, relationship: v})}
              />
            </div>
...
const FormInput = ({ label, placeholder, value, error, onChange, type = "text" }: any) => (
  <div className="space-y-2">
    ...
    <input 
      ...
      className={`w-full h-16 px-6 rounded-2xl bg-white dark:bg-white/5 border-2 transition-all outline-none text-base font-bold ${
        error ? 'border-pink-500/50 focus:border-pink-500' : 'border-slate-100 dark:border-white/5 focus:border-primary'
      }`}
    />
...
const FormSelect = ({ label, value, options, onChange }: any) => (
  <div className="space-y-2">
    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-1">{label}</label>
    <select 
      value={value}
